module Datapath(
    ...
    PC_Counter pc_counter(.clk(clk), .rst(rst), .alu_output(flag_zero), .extended(immediate), .opcode(ins[31:21]), .pc(ins_Addr));
    ...
    MUX mux_reg(.data(rReadData2), .immed(rReadData2), .flag(control_ALUSrc), .out(ALU_input2));

    instr_parse instr_par(.instr(ins), .Read_data1(rReadData1), .Read_data2(rReadData2), .immediate(immediate));
    
    ALU alu(.opcode(), .A(rReadData1), .B(ALU_input2), .out(memAddr), .zero(flag_zero));
